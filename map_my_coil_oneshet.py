...
'''
#scan along y sim
data=np.transpose(np.loadtxt('yscan.out'))
y_sim,bx_sim,by_sim,bz_sim=data
bx_sim=bx_sim*1e9 # convert to nT
by_sim=by_sim*1e9
bz_sim=bz_sim*1e9

#scan along z sim
data=np.transpose(np.loadtxt('zscan.out'))
z_sim,bx_sim,by_sim,bz_sim=data
bx_sim=bx_sim*1e9 # convert to nT
by_sim=by_sim*1e9
bz_sim=bz_sim*1e9
'''
...
'''
#scan along y target
data=np.transpose(np.loadtxt('yscan_target.out'))
y_target,bx_target,by_target,bz_target=data
bx_target=bx_target*1e9 # convert to nT
by_target=by_target*1e9
bz_target=bz_target*1e9
...
#scan along z target
data=np.transpose(np.loadtxt('yscan_target.out'))
z_target,bx_target,by_target,bz_target=data
bx_target=bx_target*1e9 # convert to nT
by_target=by_target*1e9
bz_target=bz_target*1e9
'''
...
#positions=np.mgrid[-25:26:1,0:1:1,0:1:1].reshape(3,-1).T     #change these mapping positions accordingly.

#x scan
positions=np.mgrid[-100:100:1,0:1:1,0:1:1].reshape(3,-1).T # based on the MSR (-100 cm to 100 cm scanning range)
print(positions)
'''
#y scan
positions=np.mgrid[0:1:1,-100:100:1,0:1:1].reshape(3,-1).T
#z scan
positions=np.mgrid[1:1:1,0:1:1,-100:100:1].reshape(3,-1).T
'''

# Set up a signal handler that exits if Ctrl-C is pressed
...
def set_all_voltages():
    with open('current_onesheet.csv','r') as file:
        for line in file:
